...
public class Weather extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// String zipCodeJSONURL = "http://maps.google.com/maps/geo?ll=" +
		// 37.775,-122.4183333
	...
	public void processCurrentConditions(Message m) {
		...
		wo.setTemperature(node.findElementByAttValue("class", "temp", true, false).getText().toString()
				.replaceAll("&#176;", "�"));
		...
		wo.setWind_direction(node.findElementByAttValue("id", "winddir", true, false).getText().toString()
				.replaceAll("&#176;", "�"));
		wo.setBarometer(node.findElementByAttValue("id", "pressure", true, false).getText().toString());
		wo.setDewpoint(node.findElementByAttValue("id", "dewpoint", true, false).getText().toString()
				.replaceAll("&#176;", "�"));
		wo.setHeat_index(node.findElementByAttValue("id", "heatindex", true, false).getText().toString()
				.replaceAll("&#176;", "�"));
		wo.setWind_chill(node.findElementByAttValue("id", "windchill", true, false).getText().toString()
				.replaceAll("&#176;", "�"));
	...
	public void processWeatherForecast(Message m) {
		...
		StringTokenizer tokenizer = new StringTokenizer(message.getDescription(), ".");
		String tempPattern = "[lows|highs] in the";
		String windPattern = "mph";
		String rainPattern = "rain";

		StringBuilder description = new StringBuilder();
		StringBuilder wind = new StringBuilder();

		weatherForecast.setDate(message.getTitle());

		while (tokenizer.hasMoreElements()) {
			String tokenizedString = tokenizer.nextToken();
			if (tokenizedString.indexOf(windPattern) > 0) {
				wind.append(tokenizedString);
				wind.append(".");
			} else if (tokenizedString.indexOf(rainPattern) > 0) {
				weatherForecast.setPrecipChance(tokenizedString);
			} else if (tokenizedString.matches(tempPattern)) {
				weatherForecast.setTemperature(tokenizedString);
			} else {
		...
		weatherForecast.setWind(wind.toString());
		weatherForecast.setDescription(description.toString());

		weatherObject.getForecasts().add(weatherForecast);
	}
	...
	public void updateCurrentWeatherDisplay() {
		...
		WindChillHeatIndxTextView
				.setText("Wind Chill: " + (CharSequence) wo.getWind_chill() + " Heat Index: " + wo.getHeat_index());
	...
	public void updateWeatherForecastDisplay() {
		...
		List<WeatherForecast> forecasts = weatherObject.getForecasts();
		for (int x = 0; x < forecasts.size(); x++) {
			fcDateTextView[x] = findViewById(fcDateTextViews[x]);
			fcDateTextView[x].setText(forecasts.get(x).getDate());

			fcDescTextView[x] = findViewById(fcDescTextViews[x]);
			fcDescTextView[x].setText(forecasts.get(x).getDescription());

			fcTempTextView[x] = findViewById(fcTempTextViews[x]);
			fcTempTextView[x].setText(forecasts.get(x).getTemperature());

			fcWindTextView[x] = findViewById(fcWindTextViews[x]);
			fcWindTextView[x].setText(forecasts.get(x).getWind());

			fcPrecipTextView[x] = findViewById(fcPrecipTextViews[x]);
			fcPrecipTextView[x].setText(forecasts.get(x).getPrecipChance());
		}
	}
}